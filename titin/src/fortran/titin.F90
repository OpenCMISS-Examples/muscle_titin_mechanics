...
#include "mpif.h"
  ...
  !--------------------------------------------------------------------------------------------------------------------------------
  ! With this Example the Results from Heidlauf et al 2017 can be reproduced
...
!  real etime          ! Declare the type of etime()
  ...
  ! Material Parameters
  REAL(CMISSRP), PARAMETER :: P_max=7.3_CMISSRP ! N/cm^2 -> To scale the active muscle stresses
  REAL(CMISSRP), PARAMETER :: T_max=7.3_CMISSRP ! N/cm^2 -> To scale the titin Stresses
  REAL(CMISSRP), PARAMETER :: W_ATI=1.0_CMISSRP ! 1: With Actin-Tintin Interaction 0: No Actin-Titin Interactions
  REAL(CMISSRP), PARAMETER :: W_XF=0.0_CMISSRP  ! 1: With XF-Titin Stresses 0: Without XF-Titin Stresses (See Heidlauf et al 2016)
  ! Passive Properties
  ...
  REAL(CMISSRP), PARAMETER, DIMENSION(4) :: MAT_FE= &
    ...
    & gama*9.173311371574769_CMISSRP] ! [N/cm^2]
  REAL(CMISSRP), PARAMETER :: SD=0.0_CMISSRP ! Standart Deviation (to introduce heterogenious material parameters -> See Heidlauf et al 2017) 
  REAL(CMISSRP), PARAMETER :: init_stretch=1.0_CMISSRP
  LOGICAL :: With_fv=.TRUE. ! 1: With Force-Velocity-Relation 0: Without Force-Velocity-Relation 
...
!  REAL(CMISSRP), PARAMETER :: TIME_STOP=300.0_CMISSRP
  REAL(CMISSRP), PARAMETER :: TIME_STOP=1.0_CMISSRP
  REAL(CMISSRP), PARAMETER :: TIME_STOP_2=4.0_CMISSRP !time the muscle is stimulated at fixed length + stretch
!  REAL(CMISSRP), PARAMETER :: TIME_STOP_3=1000.0_CMISSRP !time the muscle is stimulated at fixed length + stretch + fixed length
...
!tomo keep ELASTICITY_TIME_STEP and STIM_STOP at the same values
  ...
  REAL(CMISSRP), PARAMETER :: Vmax=-0.015_CMISSRP 
  ...
  INTEGER(CMISSIntg) :: stimcomponent,testcomponent
...
!##################################################################################################################################
  pathname = "./input/"
    filename = trim(pathname)//"Aliev_Panfilov_Razumova_Titin_test_example.cellml"
!  IF(With_fv) THEN
...
!  CALL cmfe_Field_ComponentInterpolationSet(MaterialFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,6,CMFE_FIELD_CONSTANT_INTERPOLATION,Err)
  ...
  CALL cmfe_Field_ComponentValuesInitialise(MaterialFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,5,0.0_CMISSRP, &
   & Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,6,T_max,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,7,W_ATI, &
   & Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,8,W_XF, &
  & Err)
...
!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.0_CMISSRP,Err) ! To change the initial half-sarcomere length. To create cases similar to Rode. 
        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*init_stretch/stretch_sarcolength_ratio,Err) ! To change the initial half-sarcomere length. To create cases similar to Rode. 
    ENDIF
  ENDDO
...
!  ENDDO
  ...
  time = time+ELASTICITY_TIME_STEP
!  time = time+PERIODD
